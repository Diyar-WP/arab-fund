...
function Careers()
{
    ...
    $data .= '
   	<h5 class="en_view">Vacancies: </h5> <h5 class="ar_view" style="direction:ltr; text-align: right">:الوظائف الشاغرة</h5>
   <div class="conferences_container">';
    while ($query->have_posts()): $query->the_post();

        $data .= '
    	<div class="conference_box">
        <div class="date_event_type">
            <div class="event_published_date">' . get_field("start_date") . ' - ' .  get_field("end_date") . '</div>
        </div>
        <h2>' . get_the_title() . '</h2>
        <div class="conference_event_details">
            <div class="event_date_from event_dates">
        ...
        <hr>
        <a href="' . get_field("url") . '" class="apply-now-anchor" data-anchor-id="p' . get_the_ID() . '" target="_blank"> <span class="ar_view">قدم طلبك الآن</span><span class="en_view">Apply Now</span> <img decoding="async" src="/wp-content/uploads/2024/01/green_arrow_event.svg" alt="readmore-icon"></a>
    </div>
    ';
    ...
    window.onload = function(){
     let allJobs =' . json_encode($query->posts) .'
    let positionField = document.getElementById("field_position_apply");
    
    allJobs.forEach((job) => {
    	let opt = document.createElement("option");
         opt.value = job.post_title;
         opt.text = job.post_title;
         opt.classList.add("position-option");
         opt.dataset.postId = "p" + job.ID;
         positionField.options.add(opt);
 
    });
    
     let anchorApply = document.querySelectorAll(".apply-now-anchor"); // this element contains more than 1 DOMs.
    for(let i =0; i < anchorApply.length; i++) {
        anchorApply[i].onclick = function(e) { 
        	selectJob(this.dataset.anchorId);
        };
    }
    
      function selectJob(jobId){
      console.log("data set",jobId);
		let optionSelected = document.querySelectorAll("[data-post-id=" + jobId + "]")[0];
...
// Custom post type for Event News
function arabfund_pressRelease_list()
{
    ob_start();

    $data = '<div class="events_container mb-5">';
    // $query = new WP_Query('post_type=news_or_activity&order=desc&posts_per_page=-1'); 
    $query = new WP_Query(array(
    ...
    while ($query->have_posts()): $query->the_post();
        $eventnewsimg = get_the_post_thumbnail_url(get_the_ID(), 'full');
        if ($eventnewsimg == "") {
            $eventnewsimg = "/wp-content/themes/hello-elementor-child/assets/images/noimg_logo.jpg";
        }
        $post_id = get_the_ID();
        $categories = get_the_terms($post_id, 'news_or_activity_type');
        $arabdate = get_field('release_date');
        if ($arabdate) {
            $to = str_replace('/', '-', $arabdate);
            //$arabevent_date = date("d-F-Y", strtotime($to));
            $day = date("d", strtotime($to));
        ...
        $data .= ' 
                          ...
                          <p class="event_description">' . get_the_excerpt() . '</p>
...
add_shortcode('arabfund_pressRelease_list_shortcode', 'arabfund_pressRelease_list'); /// end arabfund News Event Section


add_shortcode('single_press_release_shortcode', 'single_press_release');

function single_press_release()
{
    ob_start();
    ...
    $eventnewsimg = get_the_post_thumbnail_url(get_the_ID(), 'full');
    if ($eventnewsimg == "") {
        $eventnewsimg = "/wp-content/themes/hello-elementor-child/assets/images/noimg.png";
    }
    $post_id = get_the_ID();
    ...
    $data .= '<div class="content">
                                <img class="left-side-box" src="' . $eventnewsimg . '">
                                  ...
                                  <div class="news_date" style="margin-left: 0"><img src="/wp-content/uploads/2023/11/calendar_icon.svg" alt="date-icon"><span>' . $event_news_date . '</span></div>
                              </div>
                                <p>' . get_the_content() . '</p>';

    if (have_rows('video_urls', $post_id)):
        $data .= '<div class="video-gallery">';
        while (have_rows('video_urls', $post_id)) : the_row();
...
function policies_report_list()
{
    ...
    //$query = new WP_Query('post_type=policy-study&showposts=-1&order=ASC'); 
    if ($query->have_posts()) :
        while ($query->have_posts()): $query->the_post();
            $annualreportsImg = get_the_post_thumbnail_url(get_the_ID(), 'full');
            if ($annualreportsImg == "") {
            ...
            $data .= '<a class="report_box" href="' . get_field("attachment") . '" target="_blank">    
                    <img src=" ' . $annualreportsImg . ' " alt=' . get_field("annual_report_titile") . '>
                    <p class="report_body">' . get_field("study_title") . '</p>
                   </a>';
...
add_shortcode('policies_report_list_shartcode', 'policies_report_list');



function TestScriptTags()
{
    return '<script src="https://www.arabfund.org/wp-content/plugins/essential-addons-for-elementor-lite/assets/front-end/js/view/general.min.js?ver=6.0.7" id="eael-general-js" type="text/javascript"></script>
<script src="https://www.arabfund.org/wp-content/uploads/essential-addons-elementor/eael-15583.js?ver=1729627337" id="eael-15583-js" type="text/javascript"></script>
<script src="https://www.arabfund.org/wp-content/plugins/elementor/assets/js/webpack.runtime.min.js?ver=3.24.6" id="elementor-webpack-runtime-js" type="text/javascript"></script>
<script src="https://www.arabfund.org/wp-content/plugins/elementor/assets/js/frontend-modules.min.js?ver=3.24.6" id="elementor-frontend-modules-js" type="text/javascript"></script>

<script id="elementor-frontend-js-before" type="text/javascript">
var elementorFrontendConfig = {"environmentMode":{"edit":false,"wpPreview":true,"isScriptDebug":false},"i18n":{"shareOnFacebook":"Share on Facebook","shareOnTwitter":"Share on Twitter","pinIt":"Pin it","download":"Download","downloadImage":"Download image","fullscreen":"Fullscreen","zoom":"Zoom","share":"Share","playVideo":"Play Video","previous":"Previous","next":"Next","close":"Close","a11yCarouselWrapperAriaLabel":"Carousel | Horizontal scrolling: Arrow Left & Right","a11yCarouselPrevSlideMessage":"Previous slide","a11yCarouselNextSlideMessage":"Next slide","a11yCarouselFirstSlideMessage":"This is the first slide","a11yCarouselLastSlideMessage":"This is the last slide","a11yCarouselPaginationBulletMessage":"Go to slide"},"is_rtl":false,"breakpoints":{"xs":0,"sm":480,"md":768,"lg":1025,"xl":1440,"xxl":1600},"responsive":{"breakpoints":{"mobile":{"label":"Mobile Portrait","value":767,"default_value":767,"direction":"max","is_enabled":true},"mobile_extra":{"label":"Mobile Landscape","value":880,"default_value":880,"direction":"max","is_enabled":false},"tablet":{"label":"Tablet Portrait","value":1024,"default_value":1024,"direction":"max","is_enabled":true},"tablet_extra":{"label":"Tablet Landscape","value":1200,"default_value":1200,"direction":"max","is_enabled":false},"laptop":{"label":"Laptop","value":1366,"default_value":1366,"direction":"max","is_enabled":false},"widescreen":{"label":"Widescreen","value":2400,"default_value":2400,"direction":"min","is_enabled":false}},"hasCustomBreakpoints":false},"version":"3.24.6","is_static":false,"experimentalFeatures":{"e_font_icon_svg":true,"additional_custom_breakpoints":true,"container":true,"container_grid":true,"e_swiper_latest":true,"e_nested_atomic_repeaters":true,"e_onboarding":true,"theme_builder_v2":true,"hello-theme-header-footer":true,"home_screen":true,"ai-layout":true,"landing-pages":true,"nested-elements":true,"link-in-bio":true,"floating-buttons":true,"display-conditions":true,"form-submissions":true,"mega-menu":true},"urls":{"assets":"https:\/\/www.arabfund.org\/wp-content\/plugins\/elementor\/assets\/","ajaxurl":"https:\/\/www.arabfund.org\/wp-admin\/admin-ajax.php","uploadUrl":"https:\/\/www.arabfund.org\/wp-content\/uploads"},"nonces":{"floatingButtonsClickTracking":"2411f567c5"},"swiperClass":"swiper","settings":{"page":[],"editorPreferences":[]},"kit":{"active_breakpoints":["viewport_mobile","viewport_tablet"],"lightbox_enable_counter":"yes","lightbox_enable_fullscreen":"yes","lightbox_enable_zoom":"yes","lightbox_enable_share":"yes","lightbox_title_src":"title","lightbox_description_src":"description","hello_header_logo_type":"logo","hello_header_menu_layout":"horizontal","hello_footer_logo_type":"logo"},"post":{"id":15583,"title":"test%20-%20Arab%20Fund%20for%20Economic%20and%20Social%20Development","excerpt":"","featuredImage":false}};
</script>
<script src="https://www.arabfund.org/wp-content/plugins/elementor/assets/js/frontend.min.js?ver=3.24.6" id="elementor-frontend-js" type="text/javascript"></script>
';
}

add_shortcode('test_script_shorcode', 'TestScriptTags');
